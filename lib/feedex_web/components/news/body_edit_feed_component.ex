defmodule FeedexWeb.BodyEditFeedComponent do
  ...
  def update(session, socket) do
    ...
    opts =
      if register do
        feed_count =
          from(reg in Account.Register, select: count(reg.id), where: reg.feed_id == ^register.feed_id)
          |> Repo.one()
        ...
        folders = from(fld in Account.Folder,
  ...
  def render(assigns) do
    ...
    <%= if @register do %>
      ...
      >
        ...
        <.input type="select" field={{f, :folder_id}} value={@folder.id} options={@folders} label="Folder" />
        <:actions>
